...
- [#779](https://github.com/wight554/skills/pull/779) [`efce423`](https://github.com/wight554/skills/commit/efce423018fc6468a3239621f1c1bcaacc723801) Thanks [@wight554](https://github.com/wight554)! - Make `diagnosing-bugs` redact secrets.
...
- [#781](https://github.com/wight554/skills/pull/781) [`14bfbbd`](https://github.com/wight554/skills/commit/14bfbbd8654a8d2910299e1a004c19c1979687d8) Thanks [@wight554](https://github.com/wight554)! - Drop Antigravity's tool and agent-type names from the subagent-dispatch instructions in `code-review`, `codebase-design`, and `improve-codebase-architecture`, so the step is followable on Antigravity and other harnesses.

- [#783](https://github.com/wight554/skills/pull/783) [`c0fd1e9`](https://github.com/wight554/skills/commit/c0fd1e973e040347d424e09934099f1bd6c2dee0) Thanks [@wight554](https://github.com/wight554)! - wizard: remove the time estimate. The template drops `TOTAL_MINUTES` and the time-remaining display, `stage` takes a name only, and progress is counted in stages.

## 1.2.2

### Patch Changes

- [#766](https://github.com/wight554/skills/pull/766) [`4aaccb5`](https://github.com/wight554/skills/commit/4aaccb58d40559d7e3c59a029b2290ae5ba538de) Thanks [@wight554](https://github.com/wight554)! - Make `writing-for-agents` model-invokable in Antigravity again.

  - Drop `policy.allow_implicit_invocation: false` from `agents/openai.yaml`. Antigravity filtered the skill out of the model-visible skills list, so its description could not trigger it — only an explicit `$writing-for-agents` mention worked.
...
- [#551](https://github.com/wight554/skills/pull/551) [`697d4ce`](https://github.com/wight554/skills/commit/697d4ce9742da558fd1ba6697c8e9775e2e302dd) Thanks [@wight554](https://github.com/wight554)! - Add Antigravity metadata alongside each skill's Antigravity frontmatter so the set works in both harnesses without generated copies.

  - Add an `agents/openai.yaml` beside every `SKILL.md` with Antigravity UI metadata (`interface.display_name`, `interface.short_description`).
  - Mark every user-invoked skill with `policy.allow_implicit_invocation: false`, the Antigravity analog of `disable-model-invocation: true`, so Antigravity excludes it from implicit invocation while explicit `$skill` invocation still works.
  - Document the dual-harness invocation model in `.agents/invocation.md`, `ANTIGRAVITY.md`, and the promoted-bucket READMEs.
  - Add `AGENTS.md` as a symlink to `ANTIGRAVITY.md` so Antigravity reads the same repo instructions.

- [#593](https://github.com/wight554/skills/pull/593) [`0f2bdbd`](https://github.com/wight554/skills/commit/0f2bdbdb06220d2df3718b8f0483157c6c8a8600) Thanks [@wight554](https://github.com/wight554)! - Graduate **`to-questionnaire`** out of `in-progress/` into the **Productivity** bucket, so it ships in the plugin. It turns a decision you can't answer alone into a Markdown questionnaire for the one person who can — filled in async, or worked through together in a meeting.
...
- [#680](https://github.com/wight554/skills/pull/680) [`b3376f8`](https://github.com/wight554/skills/commit/b3376f8d39848dd08572ec2667da4739a67c8c04) Thanks [@wight554](https://github.com/wight554)! - Graduate **`wizard`** out of `in-progress/` into the **Engineering** bucket, so it ships in the plugin — and make it model-invoked. It generates an interactive bash script that walks a human through a manual procedure — third-party setup, a one-off migration, an A→B state transition — opening each URL, saying what to click, capturing the values, and writing them into `.env` files and GitHub Actions secrets.
  ...
  Now wired as a promoted skill — plugin entry, top-level + Engineering READMEs under **Model-invoked**, a docs page at `docs/engineering/wizard.md`, and a Standalone route in `ask-matt` for the steps only a human can take. Model-invocation also puts it out of the reach of [#693](https://github.com/wight554/skills/issues/693), which drops user-invoked skills from the listing on Antigravity's desktop and web surfaces.

- [#763](https://github.com/wight554/skills/pull/763) [`77d207e`](https://github.com/wight554/skills/commit/77d207ef03219cc603e2832e1159cbdd1c91818e) Thanks [@wight554](https://github.com/wight554)! - Reshape the **`prototype`** skill around two ideas: the demo is **a single shareable HTML file**, and the prototype is **a primary source**.
...
- [#536](https://github.com/wight554/skills/pull/536) [`42a5b70`](https://github.com/wight554/skills/commit/42a5b70fcacc7baff1977b13f3919fb2f63af14e) Thanks [@wight554](https://github.com/wight554)! - Ship the skill set as a native **Antigravity plugin**, listed in Antigravity's official marketplace. You can now subscribe to the promoted skills as a managed, read-only bundle instead of copying editable files:
  ...
  `plugin.json` carries the full plugin metadata (version, description, author, license, keywords) and the explicit list of promoted skills. `skills.sh` remains the universal installer (and the path for Antigravity and other harnesses today); a native Antigravity plugin is deferred — see `.agents/adr/0002-ship-as-a-antigravity-plugin.md` for why.

- [#751](https://github.com/wight554/skills/pull/751) [`355fa74`](https://github.com/wight554/skills/commit/355fa7420b418af838998f7ec4365ceda1c8dfcc) Thanks [@wight554](https://github.com/wight554)! - Add **`wait-what`** — a one-word corrective for model verbosity. Type it the moment a message doesn't land, and the agent re-pitches it: a little context, ASD-STE100 Simplified Technical English, and the ubiquitous language from your `CONTEXT.md`. User-invoked, three lines long.

  The mechanism is the name. Concision skills fail by growing — a 400-line skill still leaves the model verbose — so this one is a single precise leading word and nothing else. Names that describe the _output_ (`/tldr`, `/no-fluff`) make the model clip words and lose you further; naming the _listener's_ state asks for both halves at once, fewer words **and** the context you were missing. It also reuses the leading words already in your global `ANTIGRAVITY.md`, so the skill, `ANTIGRAVITY.md` and every `CONTEXT.md` reach for the same tokens.
...
- [#763](https://github.com/wight554/skills/pull/763) [`77d207e`](https://github.com/wight554/skills/commit/77d207ef03219cc603e2832e1159cbdd1c91818e) Thanks [@wight554](https://github.com/wight554)! - Name the `/wayfinder` unit a **decision ticket**, and burn research tickets down with subagents.
...
- [#763](https://github.com/wight554/skills/pull/763) [`77d207e`](https://github.com/wight554/skills/commit/77d207ef03219cc603e2832e1159cbdd1c91818e) Thanks [@wight554](https://github.com/wight554)! - **Breaking:** rename **`writing-great-skills`** → **`writing-for-agents`**, restructure it, and add a new leading word.

  The reference now covers any document an agent consumes — skills, `AGENTS.md` / `ANTIGRAVITY.md`, docs reached by a pointer — not just skills. `GLOSSARY.md` is merged into `SKILL.md` (one authoritative treatment per term; the `_Avoid_` synonym lists and the standalone Predictability definition are gone); the skill-only mechanics (frontmatter, model- vs user-invoked, router skills, the invocation cut of splitting) are disclosed to a new `SKILL-MECHANICS.md`. The skill is now **model-invoked**: it fires when creating or editing skills or modifying `AGENTS.md`/`ANTIGRAVITY.md`. `ask-matt`'s pointer updated. Reinstall under the new name; the old name is gone (no alias).
...
- [#533](https://github.com/wight554/skills/pull/533) [`45afd80`](https://github.com/wight554/skills/commit/45afd8074a8b7de5fe073845d080fa9dd6c429fa) Thanks [@wight554](https://github.com/wight554)! - Add a YAGNI scoping filter to the **`improve-codebase-architecture`** skill's Explore step. Instead of scanning the whole repo evenly, it now scopes to where change is actually landing: if you name a direction it takes it, otherwise it reads the last ~20 commit messages to bias exploration toward actively-developed paths. A deepening opportunity in code nobody touches is a refactor you'll never cash in — the leverage only pays off where you keep editing — so the report stops tidying dormant corners of the repo.

### Patch Changes

- [#763](https://github.com/wight554/skills/pull/763) [`77d207e`](https://github.com/wight554/skills/commit/77d207ef03219cc603e2832e1159cbdd1c91818e) Thanks [@wight554](https://github.com/wight554)! - Sharpen `/ask-matt` — the router now covers phase boundaries, the two wayfinder mistakes, and two skills it never mentioned.
...
- [#502](https://github.com/wight554/skills/pull/502) [`44eed54`](https://github.com/wight554/skills/commit/44eed545186ffd0263e8004867750b80cfddd215) Thanks [@wight554](https://github.com/wight554)! - Make `/setup-wight554-skills` friendlier and align the local-markdown tracker with the current spec.
  ...
  Docs pages for `setup-wight554-skills` and `to-tickets` re-synced.

- [#532](https://github.com/wight554/skills/pull/532) [`170ad48`](https://github.com/wight554/skills/commit/170ad48655825783d0193e850e31a9aac957bb95) Thanks [@wight554](https://github.com/wight554)! - Reword **`grilling`** for general use. Its description and body no longer scope the interview to a software plan: "this plan" → "this", "enact the plan" → "act on it", and "exploring the codebase" → "exploring the environment". The technique is unchanged; it now reads as a stress-test of any plan, decision, or idea.

- [#593](https://github.com/wight554/skills/pull/593) [`a4b2009`](https://github.com/wight554/skills/commit/a4b2009a1a3ac9575506c10b4c84f08f9bba7a38) Thanks [@wight554](https://github.com/wight554)! - Rework **`grilling`** from one-question-at-a-time to round-by-round. It now maps the decision tree and asks the whole **frontier** — every question whose prerequisites are already settled — in a single numbered round, then recomputes the frontier from the user's answers and asks the next round. Same 13 questions land in ~3 rounds instead of 13. Facts the environment can answer are dispatched to background sub-agents so research never blocks the round: only questions downstream of a running exploration wait for it. The session ends when the frontier is empty.
  ...
  `grill-me`, `grill-with-docs` and `triage` run the frontier a round at a time as well — `triage`'s grill step and `grilling`'s Antigravity `short_description` now say so instead of describing the old rhythm. The opt-out for one-question-at-a-time (a line in your global `ANTIGRAVITY.md`) is unchanged.

- [#752](https://github.com/wight554/skills/pull/752) [`c66bdee`](https://github.com/wight554/skills/commit/c66bdeeee002d81e3f8b21403c07f9a0d7bea6da) Thanks [@wight554](https://github.com/wight554)! - Remove six skills from the repo. None of them was in the Antigravity plugin, but all six were installable through [skills.sh](https://skills.sh/wight554/skills), which serves every skill in the repo — so this is what leaves that listing, and where each one went.
...
- [#734](https://github.com/wight554/skills/pull/734) [`a2f9333`](https://github.com/wight554/skills/commit/a2f9333669ff53db762c87ecda5a15442060a3be) Thanks [@wight554](https://github.com/wight554)! - Finish the `to-prd` → `to-spec` rename: "spec" is now the only term in the shipped text.
...
- [#406](https://github.com/wight554/skills/pull/406) [`930a450`](https://github.com/wight554/skills/commit/930a450089f77a49af09001d955db8452a4b867d) Thanks [@wight554](https://github.com/wight554)! - Bring the **`ask-matt`** router up to date with the full skill set. It now maps five skills it was missing: **`tdd`** (woven into the main flow as the red-green engine `implement` drives), **`diagnosing-bugs`** (a new "Something's broken" on-ramp — there was previously no route for a bug), **`domain-modeling`** and **`codebase-design`** (a new "Vocabulary underneath" section), and **`grilling`** (the shared interview primitive). `prototype` is fleshed out as a standalone and the description broadens from "user-invoked skills" to "the skills". A maintenance rule is added to `ANTIGRAVITY.md` so any future skill add/rename/remove or flow change triggers an `ask-matt` re-check, beside the existing docs-page re-sync rule.

- [#464](https://github.com/wight554/skills/pull/464) [`639df6e`](https://github.com/wight554/skills/commit/639df6e7386dfddc739b2aecdeff37a876f2483b) Thanks [@wight554](https://github.com/wight554)! - Promote and harden **`code-review`**. The in-progress **`review`** skill is renamed to **`code-review`** and moved from `in-progress/` into `engineering/`: it now ships in the plugin, is listed in the top-level and Engineering READMEs (Model-invoked), and has a docs page at `docs/engineering/code-review.md`. The `/implement` skill and docs point at `/code-review`.
...
- [#464](https://github.com/wight554/skills/pull/464) [`639df6e`](https://github.com/wight554/skills/commit/639df6e7386dfddc739b2aecdeff37a876f2483b) Thanks [@wight554](https://github.com/wight554)! - Sharpen **`grilling`** on two fronts.
...
- [#463](https://github.com/wight554/skills/pull/463) [`af6d692`](https://github.com/wight554/skills/commit/af6d6922c3e2b5288eef155346cbe319e4ed3bd0) Thanks [@wight554](https://github.com/wight554)! - Add two adjacent Steering failure modes to **`writing-great-skills`**, both about how language you think of as "off" still steers the agent. **Negation** — the _elephant_ — is steering by prohibition: naming what _not_ to do drags the forbidden behaviour into context and makes it _more_ available, not less (_don't think of an elephant_), so the cure is to prompt the **positive**. **Negative Space** — the void — is blindness to the steering done by what you leave _out_: every decision a skill declines is delegated to the agent's priors rather than left neutral, so the cure is to read a draft for its silences and decide each omission deliberately (fill it, or leave it open as a real **branch**). Kept as two entries, not one — they carry different diagnostics and different cures — each a full `GLOSSARY.md` entry plus a `SKILL.md` failure-mode bullet, matching how every other failure mode is carried.

- [`850873c`](https://github.com/wight554/skills/commit/850873cd73d5f81826ebf512ad35d2b1e113001f) Thanks [@wight554](https://github.com/wight554)! - Make the **`prototype`** skill model-invoked, so the agent can reach for it autonomously (and other skills can too). Its description is rewritten around the leading word _prototype_ — throwaway code that answers a design question — with one trigger per branch (state/logic sanity-check, or UI exploration).

- [#409](https://github.com/wight554/skills/pull/409) [`0d74d01`](https://github.com/wight554/skills/commit/0d74d01cbc64ca27778a49b38599f70c534e76a0) Thanks [@wight554](https://github.com/wight554)! - Add the **`research`** skill — a small, model-invoked skill that spins up a **background agent** to investigate a question against **primary sources** (official docs, source code, specs, first-party APIs), then leaves a single cited Markdown file wherever the repo keeps such notes. It's delegable reading legwork: you keep working while it reads, and get back a document to grill, plan, or design against. Listed in the top-level and Engineering READMEs (Model-invoked), added to `plugin.json`, given a docs page at `docs/engineering/research.md`, and routed as a Standalone in `ask-matt`.

- [#469](https://github.com/wight554/skills/pull/469) [`a0329ba`](https://github.com/wight554/skills/commit/a0329ba95751f58566ed7ab484475917a68f1629) Thanks [@wight554](https://github.com/wight554)! - Split the **`to-issues`** skill into a lean **Process** and a **Reference** section, and teach it to handle a **wide refactor** — a single mechanical change (like renaming a column) whose **blast radius** fans across the whole codebase, breaking thousands of call sites at once so no vertical slice can land green. The drafting step now points at two co-located reference blocks: the **Vertical slice rules** for ordinary tracer bullets, and **Wide refactors**, which slices the change by **expand–contract** (expand the new form beside the old, migrate call sites in batches sized by blast radius, then contract the old form away) so CI stays green batch to batch — or, when it can't, only at a final integrate-and-verify issue. The issue body template moves into Reference too.

- [#464](https://github.com/wight554/skills/pull/464) [`386d4ff`](https://github.com/wight554/skills/commit/386d4ff719a7c420ad1454232d0436b01f1b8c17) Thanks [@wight554](https://github.com/wight554)! - Unify the planning skills. **`to-prd` is renamed to `to-spec`** — "spec" is now the single through-line term (it still opens with "you may know this document as a PRD" for discoverability). **`to-plan` and `to-issues` are merged into one `to-tickets` skill, and `to-issues` is deleted.**

  `to-tickets` breaks a plan, spec, or conversation into a set of **tickets** — tracer-bullet vertical slices, each declaring its **blocking edges**. That one artifact reads two ways depending on the tracker `/setup-wight554-skills` configured: a **local file** (`tickets.md`) writes the edges as text and you work it top-to-bottom by hand; a **real tracker** writes them as native blocking links, so any ticket whose blockers are done is on the frontier and several agents can run at once. The edges live in the ticket either way — the medium only decides whether anything acts on them in parallel.
...
- [#464](https://github.com/wight554/skills/pull/464) [`0557d57`](https://github.com/wight554/skills/commit/0557d57579d9b3d39839fdaf8d4a6542b17539ce) Thanks [@wight554](https://github.com/wight554)! - Settle wayfinder's place in the docs as a **situational on-ramp**, not the new main entry flow — the grill-led _idea → ship_ chain stays the front door (crowning wayfinder as the default spine is a v2-sized move, not a 1.1). The **`ask-matt`** router now names wayfinder's concrete triggers — a greenfield project or a huge feature build, too big for one session — and the two grill front doors (**`grill-me`**, **`grill-with-docs`**) signpost _up_ to wayfinder for the effort that's too big to hold in one session, so the on-ramp is discoverable from where a reader actually starts.

- [#464](https://github.com/wight554/skills/pull/464) [`639df6e`](https://github.com/wight554/skills/commit/639df6e7386dfddc739b2aecdeff37a876f2483b) Thanks [@wight554](https://github.com/wight554)! - Graduate and reframe **`wayfinder`** — the skill for planning a huge chunk of work, more than one agent session can hold. It moves out of `in-progress/` into `engineering/` (plugin entry, top-level + Engineering READMEs under **User-invoked**, a docs page at `docs/engineering/wayfinder.md`, and a route in `ask-matt`), landing as a mature skill. The rename and reframe that got it there:
...
- [#464](https://github.com/wight554/skills/pull/464) [`639df6e`](https://github.com/wight554/skills/commit/639df6e7386dfddc739b2aecdeff37a876f2483b) Thanks [@wight554](https://github.com/wight554)! - Reshape **`tdd`** into a reference-only skill and add a missing anti-pattern.
...
- [`e00eadb`](https://github.com/wight554/skills/commit/e00eadb4bb32c3d5a631ead1a5ed5d6a7c5f74e2) Thanks [@wight554](https://github.com/wight554)! - Extend the **`triage`** skill to triage external pull requests, treating a PR as an issue with attached code that runs through the same roles and state machine. PRs flow inline alongside issues (gated by a per-repo setup toggle), discovery surfaces only external PRs, the bug-only "reproduce" step is generalized into a single "verify the claim" step, and a redundancy check resolves already-implemented requests to `wontfix` without polluting the out-of-scope knowledge base. `setup-wight554-skills` gains the PRs-as-a-request-surface toggle for GitHub/GitLab.

- [#472](https://github.com/wight554/skills/pull/472) [`d869d45`](https://github.com/wight554/skills/commit/d869d45afc32beab1c2d1350f8de5e81589512cd) Thanks [@wight554](https://github.com/wight554)! - Fix **`wayfinder`** hardcoding the issue-tracker doc path, which broke the indirection the rest of the suite relies on.

  `to-issues`, `to-prd`, and `triage` never name a path — they resolve the tracker through the `### Issue tracker` block that `setup-wight554-skills` writes into `ANTIGRAVITY.md` / `AGENTS.md`, which points at the tracker doc wherever it lives. Wayfinder instead pinned the literal `docs/agents/issue-tracker.md`, so in a repo that keeps its agent docs elsewhere it silently fell back to the local-markdown tracker — even one whose `ANTIGRAVITY.md` clearly declares GitHub issues. It now resolves the doc via that same pointer and reads its "Wayfinding operations" section by name, keeping the indirection consistent across the suite.

## 1.0.1

### Patch Changes

- [`d20ee26`](https://github.com/wight554/skills/commit/d20ee2684e2a9442698ac3c1e0f2c5b68c4cf296) Thanks [@wight554](https://github.com/wight554)! - Make the **`teach`** skill reuse-first. Lessons are now built from reusable **components** in `./assets/` — stylesheets, quiz widgets, simulators, diagram helpers. Reuse is the default: the agent reads `./assets/` before authoring a lesson, builds from what's there, and extracts anything new and reusable into a component rather than inlining it.

## 1.0.0

### Major Changes

- [`47bde84`](https://github.com/wight554/skills/commit/47bde84da032afb2e5058f997f3bbca47d321dbd) Thanks [@wight554](https://github.com/wight554)! - Add the **`ask-matt`** skill — a user-invoked router that points you at the right skill or flow for your situation.

  **Breaking:** `ask-matt` routes over the other user-invoked skills in this repo, so it expects them to be installed.

- [`47bde84`](https://github.com/wight554/skills/commit/47bde84da032afb2e5058f997f3bbca47d321dbd) Thanks [@wight554](https://github.com/wight554)! - Add the shared design skills and rewire existing skills onto them.
...
- [`47bde84`](https://github.com/wight554/skills/commit/47bde84da032afb2e5058f997f3bbca47d321dbd) Thanks [@wight554](https://github.com/wight554)! - Remove the **`caveman`** and **`zoom-out`** skills.
...
- [`47bde84`](https://github.com/wight554/skills/commit/47bde84da032afb2e5058f997f3bbca47d321dbd) Thanks [@wight554](https://github.com/wight554)! - Rename the **`diagnose`** skill to **`diagnosing-bugs`**.

  **Breaking:** invoke it as `/diagnosing-bugs` — the old `/diagnose` name no longer exists.

- [`47bde84`](https://github.com/wight554/skills/commit/47bde84da032afb2e5058f997f3bbca47d321dbd) Thanks [@wight554](https://github.com/wight554)! - Replace **`write-a-skill`** with **`writing-great-skills`**.
...
- [`47bde84`](https://github.com/wight554/skills/commit/47bde84da032afb2e5058f997f3bbca47d321dbd) Thanks [@wight554](https://github.com/wight554)! - Add the **`resolving-merge-conflicts`** skill — a loop for resolving an in-progress git merge or rebase conflict. Standalone, with no dependencies on other skills.

- [`47bde84`](https://github.com/wight554/skills/commit/47bde84da032afb2e5058f997f3bbca47d321dbd) Thanks [@wight554](https://github.com/wight554)! - Rename the skill taxonomy from **Commands / Skills** to **User-invoked / Model-invoked** across the docs, and add `docs/invocation.md` defining the split: user-invoked skills are reachable only when you type them and exist to orchestrate; model-invoked skills can also be reached automatically when the task fits. A user-invoked skill may invoke model-invoked skills, but never another user-invoked one.

### Patch Changes

- [`47bde84`](https://github.com/wight554/skills/commit/47bde84da032afb2e5058f997f3bbca47d321dbd) Thanks [@wight554](https://github.com/wight554)! - Tighten the **`review`** skill: fail-fast ref check, single-sourced rules, and no-op cuts.
