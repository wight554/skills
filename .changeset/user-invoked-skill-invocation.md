...
- `to-spec`, `wayfinder`, `to-tickets`, `triage`, and `code-review` each carried a precondition ("...run `/setup-wight554-skills` if not") that PR #878 rewrote into a literal `Call the Skill tool with "setup-wight554-skills"` instruction. `setup-wight554-skills` is user-invoked, so none of these skills (user-invoked or model-invoked) can call it. Reworded all five as instructions for the agent to tell the human to run it instead.
