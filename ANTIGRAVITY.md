...
Install commands are copied verbatim from [.agents/install-block.md](./.agents/install-block.md). `.antigravity-plugin/marketplace.json` makes the repo its own single-plugin marketplace (a fallback the install block explains, not the documented route). Run `antigravity plugin validate . --strict` after touching either manifest. Why a Antigravity plugin but not (yet) a Antigravity one lives in [.agents/adr/0002-ship-as-a-antigravity-plugin.md](./.agents/adr/0002-ship-as-a-antigravity-plugin.md).
...
To (re)link every skill outside `deprecated/` and `misc/` into the local harness skill directories (`~/.antigravity/skills`, `~/.agents/skills`), run `scripts/link-skills.sh`. Each entry is a symlink into this repo, so a `git pull` keeps installed skills current; re-run the script after adding, removing, or renaming a skill.
